 ...
 * `false' otherwise.
 */
static bool is_invalid_connection( ConnectionData* cd );

static void print_connections(std::vector<ConnectionData*> connections);
...
#	endif
	...
	struct fd_set listen_set, conn_set;
	FD_ZERO(&conn_set);
	...
	while( 1 )
	{
		...
		int result = select(max_fd, &listen_set, NULL, NULL, &timeout);
		...
		if(result > 0 && FD_ISSET(listenfd, &listen_set))
		{		
	...
	#			if NONBLOCKING
			...
			if( !set_socket_nonblocking( clientfd ) ){
				printf("Error setting socket as NONBLOCKING\n");
				close( clientfd );
				return -1;
			}
	#			endif

			//FD_SET(clientfd, &conn_set);
			max_fd = std::max(max_fd, clientfd) + 1;
		...
		{
			ConnectionData iconn = *connections[s];
			/*printf( "Connection %zu: in state %d and has socket %d\n",
			s, iconn.state, iconn.sock );*/
			...
			while( processFurther )
			{
				...
				if( iconn.state == eConnStateReceiving ) {
					FD_SET(iconn.sock, &conn_set);
					/*
					for(int j=0; j<max_fd; j++) {
						printf("fd: %d\tISSET: %d\n", j, FD_ISSET(j, &conn_set));
					}
					*/
					result = select(max_fd, &conn_set, NULL, NULL, &timeout);
					//printf("Found %d read sockets\n", result);
					if (result < 0)
					{
						perror("  select() failed");
						break;
					} else if(FD_ISSET(iconn.sock, &conn_set)) {
						//printf("Found a reading socket\n");
						processFurther = process_client_recv( iconn );
					} else {
						break;
					}
				}
				...
				//print_connections(connections);
				if( iconn.state == eConnStateSending ) {
					FD_SET(iconn.sock, &conn_set);
					result = select(max_fd, NULL, &conn_set, NULL, &timeout);
					//printf("Found %d write sockets\n", result);
					if (result < 0)
					{
						perror("  select() failed");
						break;
						...
						//printf("Found a writing socket\n");
						processFurther = process_client_send( iconn );
					} else {
						break;
					}
				}
				...
				if(processFurther == false) {
					...
					close( iconn.sock );
					FD_CLR(iconn.sock, &conn_set);		
					connections[s]->sock = -1;
				}
			}
		}
		//print_connections(connections);
