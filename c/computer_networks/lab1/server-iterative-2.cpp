...
#	endif
	...
	while( 1 )
	{
		...
		int result = select(max_fd, &listen_set, NULL, NULL, NULL);
	...
	#			endif
			...
			max_fd = std::max(listenfd, clientfd) + 1;
			...
			while( processFurther )
			{
				...
				if( connData.state == eConnStateReceiving ) {
					result = select(max_fd, &conn_set, NULL, NULL, NULL);
					...
					else if(FD_ISSET(clientfd, &conn_set)) {
				...
				if( connData.state == eConnStateSending ) {
					result = select(max_fd, NULL, &conn_set, NULL, NULL);
