...
#	endif
	...
	while( 1 )
	{
		...
		for( size_t s = 0; s < connections.size(); ++s ) {
			...
			if(iconn.sock >= max_fd)
				max_fd = iconn.sock + 1;
			if(iconn.state == eConnStateReceiving)
				FD_SET(iconn.sock, &readfds);
			else if(iconn.state == eConnStateSending)
				FD_SET(iconn.sock, &writefds);
		}
		//printf("max_fd: %d\n", max_fd);
		...
		printf("%d\n", max_fd);
		int ret = select( max_fd, &readfds, &writefds, 0, 0 );
		//printf("Got %d connections from select\n", ret);
...
#			endif
		...
		for( size_t s = 0; s < connections.size(); ++s ) {
			...
			//printf("Sock %d isset -> %d\n", iconn.sock, FD_ISSET(iconn.sock, &readfds));
			if(FD_ISSET(iconn.sock, &readfds)) {
				processFurther = process_client_recv(connections[s]);
				if(processFurther == false) {
					// done - close connection
					close(iconn.sock);
					FD_CLR(iconn.sock, &readfds);		
					connections[s]->sock = -1;
				}
				//printf("ConnectionState: %d\n", connections[s]->state);
			}
			if(FD_ISSET(iconn.sock, &writefds)) {
				processFurther = process_client_send(connections[s]);
				if(processFurther == false) {
					// done - close connection
					close(iconn.sock);
					FD_CLR(iconn.sock, &writefds);		
					connections[s]->sock = -1;
				}
			}
		}
