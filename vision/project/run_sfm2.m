function run_sfm(data_index)
    ...
    for loop_index = 1:length(img_names) - 1
        ...
        [x_nor{loop_index}, ~] = match_images(image_features, image_descriptors, loop_index, loop_index + 1, K);

        
        % Parallel RANSAC
        E = E_and_H_RANSAC(x_nor{loop_index}{1}, x_nor{loop_index}{2}, epipolar_threshold, homography_threshold);
        [~, P] = get_P_3D_points(x_nor{loop_index}, E);
    ...
    [x_nor_init, desc_X] = match_images(image_features, image_descriptors, im1_index, im2_index, K);

    % Estimate E and eliminate outliesr before estimating 3D-points.
    %E = estimate_E_robust(K, K*x_nor_init{1}, K*x_nor_init{2}, epipolar_threshold);
    ...
    [X_est, ~] = get_P_3D_points(x_nor_init, E);
...
function [bool, acceptable_E] = acceptable_E_from_H(x, H, err_threshold_E)
    ...
    for j = 1:2
        ...
        for i = 1:4
            P{2} = P2{i};
            X_est = pflat(triangulate_3D_point_DLT(P{1}, P{2}, filtered_x1, filtered_x2));
            x_proj1 = P{1}*X_est;
            x_proj2 = P{2}*X_est;
